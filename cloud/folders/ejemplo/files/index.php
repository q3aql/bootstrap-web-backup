...
$pa = htmlentities($_POST["p"]);
$pass = md5($pa);

session_start();
if($_SESSION['_sfm_allowed']!==$PASSWORD) {
	// sha1, and random bytes to thwart timing attacks.  Not meant as secure hashing.
	$t = bin2hex(openssl_random_pseudo_bytes(10));	
	if($pass && sha1($t.$pass) === sha1($t.$PASSWORD)) {
		$_SESSION['_sfm_allowed'] = $PASSWORD;
		header('Location: ?');
	}
	include('../../../user.html'); 
	exit;
...
# deleted to see download folder .is_dir .download{visibility: hidden}
...
<div id="top">
   <?php if($allow_upload == true): ?>
	<form action="?" method="post" id="mkdir" />
		...
		<input class="btn btn-default btn-sm" type="submit" value="create" />
	</form>
   ...
   <?php if($allow_upload == true): ?>
		...
		<input class="btn btn-default" type="file" multiple />
